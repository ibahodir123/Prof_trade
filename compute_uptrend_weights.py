#!/usr/bin/env python3
"""Compute z-score weights for uptrend profiles using shared settings."""
...
PROFILES_CSV = summary_path("upward_profiles.csv")
OUTPUT_JSON = summary_path("uptrend_weights.json")
...
def compute_weights() -> dict:
    if not PROFILES_CSV.exists():
        raise FileNotFoundError("upward_profiles.csv not found. Run extract_upward_profiles.py first.")

    profiles = pd.read_csv(PROFILES_CSV)
    uptrend = profiles[profiles["trend"] == "uptrend"].copy()

    if uptrend.empty:
        raise RuntimeError("No uptrend profiles found in dataset.")

    stats: dict[str, dict[str, float]] = {}
    for feature in PROFILE_FEATURES:
        mean = uptrend[feature].mean()
        std = uptrend[feature].std(ddof=0)
...
def main() -> None:
    ...
    print(f"Computed weights for {len(result['profiles'])} uptrend profiles -> {OUTPUT_JSON}")
