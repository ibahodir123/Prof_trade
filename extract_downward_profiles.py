#!/usr/bin/env python3
"""Extract downward impulse profiles (speed, distance, angle, length) using shared settings."""
...
OUTPUT_CSV = summary_path("downward_profiles.csv")
OUTPUT_JSON = summary_path("downward_profiles_summary.json")
...
def compute_trend_and_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["ema20_change_5"] = (df["ema_20"] - df["ema_20"].shift(5)) / df["ema_20"].shift(5)
...
def build_profiles(df: pd.DataFrame, segments: List[Segment]) -> pd.DataFrame:
        ...
        records.append(
            {
                ...
                "ema20_change_5": row["ema20_change_5"],
            }
        )
...
def summarize_profiles(df_profiles: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    ...
    for trend in [TREND_DOWN, TREND_UP, TREND_SIDE]:
...
def main() -> None:
    ...
    segments = find_downward_segments(price_df)
