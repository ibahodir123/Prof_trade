...
"""
🔺 ЧИСТЫЙ EMA ТРЕНЕР МАКСИМУМОВ
ТОЛЬКО EMA-признаки, без объема и RSI
"""
...
class PureEMAMaximumTrainer:
    def __init__(self):
        ...
        print("🔺 ЧИСТЫЙ EMA ТРЕНЕР МАКСИМУМОВ")
        print("📊 ТОЛЬКО EMA-признаки (без объема и RSI)")
        print("=" * 50)
    ...
    def prepare_pure_ema_features(self, df: pd.DataFrame, max_idx: int) -> dict:
        ...
        try:
            ...
            features = {
                ...
                # Соотношения EMA
                'ema20_ema50_ratio': ema20_ema50_ratio,
                'ema50_ema100_ratio': ema50_ema100_ratio,
                'ema20_ema100_ratio': ema20_ema100_ratio
            }
    ...
    def find_pure_ema_maximums(self, df: pd.DataFrame, symbol: str):
        """Поиск чистых EMA максимумов"""
        ...
        for i in range(50, len(df) - 6):
            try:
                ...
                if (drop_percent <= -self.min_drop_threshold and 
                    ...
                    features = self.prepare_pure_ema_features(df, i)
                    
                    if features:
                        # Проверяем качество признаков (только EMA)
    ...
    def collect_pure_ema_data(self):
        """Сбор чистых EMA данных"""
        print("\n📚 СБОР ЧИСТЫХ EMA ДАННЫХ МАКСИМУМОВ")
        print("-" * 40)
        ...
        for i, symbol in enumerate(self.symbols):
            ...
            if not df.empty:
                self.find_pure_ema_maximums(df, symbol)
            time.sleep(0.5)
    ...
    def analyze_pure_ema_patterns(self):
        """Анализ чистых EMA закономерностей"""
        print("\n🔍 АНАЛИЗ ЧИСТЫХ EMA ЗАКОНОМЕРНОСТЕЙ")
        print("-" * 40)
        ...
        print("🏆 ВАЖНОСТЬ ПРИЗНАКОВ (только EMA):")
    ...
    def train_pure_ema_model(self):
        """Обучение чистой EMA модели"""
        print("\n🧠 ОБУЧЕНИЕ ЧИСТОЙ EMA МОДЕЛИ")
        print("-" * 30)
        ...
        print(f"\n🏆 ВАЖНОСТЬ ПРИЗНАКОВ (чистая EMA модель):")
        for name, importance in feature_importance:
            print(f"   {name}: {importance:.4f}")
        
        # Сохраняем модель
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        model_filename = f'pure_ema_maximum_model_{timestamp}.pkl'
        feature_filename = f'pure_ema_maximum_features_{timestamp}.pkl'
        ...
        print(f"\n💾 Чистая EMA модель сохранена: {model_filename}")
        print(f"💾 Признаки сохранены: {feature_filename}")
        
        return model, feature_names
    
    def run_pure_ema_analysis(self):
        """Запуск чистого EMA анализа"""
        print("🚀 ЧИСТЫЙ EMA АНАЛИЗ МАКСИМУМОВ")
        print("=" * 50)
        
        # 1. Сбор данных
        self.collect_pure_ema_data()
        ...
        self.analyze_pure_ema_patterns()
        
        # 3. Обучение модели
        model_result = self.train_pure_ema_model()
        
        print(f"\n✅ ЧИСТЫЙ EMA АНАЛИЗ ЗАВЕРШЕН!")
        print(f"📊 Найдено {len(self.maximums_database)} максимумов")
        
        if model_result:
            print(f"🧠 Чистая EMA модель обучена и готова к использованию")

if __name__ == "__main__":
    trainer = PureEMAMaximumTrainer()
    trainer.run_pure_ema_analysis()
